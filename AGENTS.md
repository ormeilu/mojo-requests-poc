...
- **`AF_INET6` is NOT a portable constant** — it's `30` on macOS/BSD and `10` on Linux (unlike `AF_INET=2`, which is fixed). Never declare it as a `comptime c_int`. For IPv6 literal parsing use `getaddrinfo(host, AI_NUMERICHOST)` (it reports the correct per-platform `ai_family`); for `socket()`/`sin6_family`, carry the raw `ai_family` in `ResolvedAddress.pf` and pass that. Family detection compares against the portable `AF_INET` and treats anything else as IPv6. (See STRUGGLES.md §5.6.)
