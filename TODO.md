...
- [ ] **TLS performance tuning** — OpenSSL is CPU-bound; most of the cost is the handshake. Three levers, in order of payoff:
  - [x] **TLS session resumption** (highest payoff). Implemented in [`requests/_tls.mojo`](requests/_tls.mojo) (`TLSSessionCache`) + `Session._connect_new`. `SSL_CTX_set_session_cache_mode(ctx, SSL_SESS_CACHE_CLIENT)` is enabled per-context (via `SSL_CTX_ctrl` — the setter is a macro, not an exported symbol, same trap as `SSL_set_tlsext_host_name`/`SSL_CTX_set_min_proto_version`). `Session` owns a `TLSSessionCache` (host:port → `SSL_SESSION*`, freed via `SSL_SESSION_free` on eviction/replace/drop). On each fresh HTTPS connect, a cached session for that endpoint (if any) is offered via `SSL_set_session` before the handshake; after the handshake, `SSL_get1_session` retrieves the (possibly resumed, possibly new) session and restashes it. Best-effort throughout — a rejected/expired session just falls back to a full handshake. Only wired into the non-proxied keep-alive path (`_connect_new`); `_connect_proxied` and the streaming path don't participate yet (future work). While a `KeptAliveConn` stays pooled the handshake is skipped entirely (the socket is reused); resumption specifically helps the *re-connect* case (stale-pool retry, or a new endpoint after eviction).
  - [x] **Cipher/protocol curation.** Implemented in [`requests/_tls.mojo`](requests/_tls.mojo)`TLSConnection.connect()`: `SSL_CTX_set_min_proto_version(TLS1_2_VERSION)` floors out SSLv3/TLS1.0/1.1 (downgrade exposure) without forcing TLS 1.3-only (would break handshakes against any still-TLS1.2-only server). `SSL_CTX_set_ciphersuites()` curates the TLS 1.3 suite list (AES-GCM/ChaCha20-Poly1305) and `SSL_CTX_set_cipher_list()` curates the TLS 1.2 ECDHE+AEAD suite list (the legacy API — `set_ciphersuites` only governs 1.3). Both calls are best-effort (return value ignored) so an older libssl that doesn't recognize a suite name still falls through to its defaults rather than failing the connection.
